"""Static release-asset validation for the Mitra Classifier DIMER pipeline.
...
generator parity checks (PAR1–PAR3) for every generated standalone notebook. Explicitly allowlisted auxiliary workshop notebooks receive separate syntax, provenance, and registry checks without weakening generated-notebook parity.
...
Generated-pair variant of the fleet validator (snapshot resnet50 @ 6c77f84, tooling updates 2026-09-13 15:40 + 17:10):
the constants block declares NOTEBOOKS (one entry per generated notebook: template, profile, gates, outputs,
markers) plus AUXILIARY_NOTEBOOKS for explicitly reviewed hand-authored workshop carriers, PACKAGE_DIR (root-level package), IDENTITY_DOCS and the card-structure toggle; the shared block is the
...
PACKAGE_DIR = "mitra_pipeline"  # root-level package (the template's package_dir)
REPO_NAME = "mitra-classifier-pipeline"
EXPECTED_MODEL_ID = "autogluon/mitra-classifier"
...
# MODEL_CARD_SPEC version the card on this branch declares; this card predates the spec's heading structure and
# front-matter field (its sections are the repository's own `##` layout, provenance under "Checkpoint and Artifact
# Provenance"), so the structural checks are switched off here and only front matter, single H1, placeholder /
# claim hygiene and the provenance section are enforced until the fleet's card pass reaches this branch.
EXPECTED_CARD_SPEC = None
VALIDATE_CARD_STRUCTURE = False
PROVENANCE_HEADING = "## Checkpoint and Artifact Provenance"
...
NOTEBOOKS = {
    "mitra_classifier_colab.ipynb": {
        ...
        "expected_outputs": (
            "outputs/mitra_classifier_input_manifest.json",
            "outputs/mitra_classifier_evaluation_report.json",
            "outputs/mitra_classifier_result.json",
            "outputs/mitra_classifier_predictions.csv",
        ),
        "code_markers": (
            "DATA_SOURCE = 'Sample: Breast Cancer'",
            ...
            "pipe.fit(train_data, target_column=TARGET_COLUMN, eval_metric=EVAL_METRIC, path=PRETRAINED_PATH, fine_tune=False",
            "MIN_SELECTION_HOLDOUT_ROWS = 50",
            "raise RuntimeError('Fine-tuning requires a GPU. Choose Runtime -> Change runtime type -> GPU.')",
            "candidate.fit(train_data, target_column=TARGET_COLUMN, eval_metric=EVAL_METRIC, path=FINETUNED_PATH, fine_tune=True, fine_tune_steps=FINE_TUNE_STEPS",
            "baseline_rows = [{'model': 'Majority-class', 'split': 'holdout', **baseline}]",
            "LGBMClassifier(random_state=SEED, n_estimators=100, verbose=-1)",
            "RandomForestClassifier(random_state=SEED, n_estimators=100)",
            "report = evaluation_report(active_metrics, baseline=baseline, independent_test=active_test_metrics, n_holdout=len(holdout_data)",
            ...
            "importlib.metadata.version('autogluon.tabular')",
        ),
        "markdown_markers": (
            "**Capability:** end-to-end tabular classification with the pinned `autogluon/mitra-classifier` checkpoint",
            "no weight is gradient-updated",
            "**not calibrated probabilities**",
            "**dropped and counted**",
            "**in the predictor's class order**",
            "**Fine-tuning gate (off by default; GPU only).**",
            "**Reproducibility boundary.**",
            "the verdict would be `not-measurable`",
            "`sample-sanity`",
            "before** deserialising",
            "regression, forecasting, calibrated probabilities, or any deployment threshold",
        ),
    },
    "mitra_classifier_predictor_inference_colab.ipynb": {
        "template": "notebook_template_artifact_inference",
        "profile": "ARTIFACT-INFERENCE",
        "byod_gates": ("ALLOW_UNVERIFIED_ARTIFACT",),
        "expected_outputs": (
            "outputs/mitra_classifier_predictor_inference_input_manifest.json",
            "outputs/mitra_classifier_predictor_inference_evaluation_report.json",
            "outputs/mitra_classifier_predictor_inference_result.json",
            "outputs/mitra_classifier_predictor_inference_predictions.csv",
        ),
        "code_markers": (
            "ARTIFACT_ZIP_PATH = ''",
            "EXPECTED_ZIP_SHA256 = ''",
            "NEW_DATA_PATH = ''",
            "raise RuntimeError('A trusted whole-archive SHA-256 is required before loading this Python-serialized artifact.",
            "safe_extract_archive(zip_path, EXTRACT_ROOT)",
            "artifact_manifest, run_metadata = validate_artifact_directory(EXTRACT_ROOT)",
            "!= (MODEL_ID, MODEL_REVISION, WEIGHTS_SHA256, CONFIG_SHA256):",
            ...
            "raise RuntimeError('Reconstructed class labels disagree with the validated provenance.')",
            ...
            "X_new, extra_columns = validate_inference_frame(new_data, FEATURE_COLUMNS)",
            "proba = serving.predict_proba(X_new)",
        ...
        "markdown_markers": (
            "**Capability:** consume an externally supplied AutoGluon predictor bundle",
            "produced **outside this execution**",
            "**No artifact is created here**",
            "**Trust boundary.**",
            "`TabularPredictor.load()` executes trusted",
            "**the probabilities are uncalibrated and no threshold is shipped by this pipeline**",
            "its verdict is `not-measurable`",
            "artifact creation, in-notebook fitting or fine-tuning, regression",
        ),
        "forbidden_code": (
            "load_breast_cancer(",
            "load_wine(",
...
NOTEBOOK_SPEC = "2.0"
ALLOWED_PROFILES = {"E2E", "ARTIFACT-INFERENCE", "TASK-INFERENCE", "MULTI-CAPABILITY", "SMOKE", "WORKSHOP"}
...
def validate_model_card() -> None:
    ...
    for key in ("license:", "base_model:") + (("model_card_spec:",) if EXPECTED_CARD_SPEC is not None else ()):
        _check(key in front, f"MODEL_CARD.md missing front-matter field: {key}")
    if EXPECTED_CARD_SPEC is not None:
    ...
    if not VALIDATE_CARD_STRUCTURE:
...
def _validate_auxiliary_workshop(path: Path, notebook: dict, spec: dict, registry: str) -> None:
    """Static checks for an explicitly allowlisted, hand-authored multi-repository workshop."""
    _check(notebook.get("nbformat") == 4, f"{path.name}: nbformat must be 4")
...
def validate_all() -> list[str]:
    ...
    validate_notebooks()
    return ["model-card", "identity-consistency", "weight-facts", "release-status", "generated-notebooks+parity", "auxiliary-workshop"]
