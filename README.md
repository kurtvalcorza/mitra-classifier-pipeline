...
`tutorials/mitra_classifier_colab.ipynb` (`E2E`) and its companion `tutorials/mitra_classifier_predictor_inference_colab.ipynb` (`ARTIFACT-INFERENCE`) are declared under DIMER Notebook Specification 1.1 and are **standalone** (§3.6): generated by `tools/build_notebook.py`, each carries the public tutorial API module `mitra_pipeline/tutorial_api.py` verbatim, the model identity (`autogluon/mitra-classifier` at immutable revision `c425e9fa0910a6be1c494321792e7ba2a1367b1a`), the snapshot manifest digests (`model.safetensors` `e06a055e91a3baeffc37f9cf634d9e69a27d904b6686131dc3b702f9c0126b19`) and the exact `pyproject.toml` runtime pins, so the exported notebooks run without this repository or DIMER Workbench (parity enforced by `tests/test_notebook_parity.py`, `tests/test_companion_parity.py` and `tools/validate_release_assets.py`). The E2E default path loads scikit-learn's bundled Breast Cancer Wisconsin table (no download; Wine is the three-class alternative), validates it with `validate_inputs` into an input manifest with class coverage preserved, registers the pretrained model through AutoGluon, evaluates it on a stratified holdout and an independent test partition against `majority_class_baseline`, LightGBM and Random Forest with strict label alignment, writes an `evaluation_report` (`sample-sanity`), optionally fine-tunes on a GPU (`RUN_FINE_TUNING`, off by default), optionally scores an uploaded CSV with class probabilities under an explicit `argmax` rule, exports the deployable AutoGluon predictor bundle with its manifest and provenance and proves it reloads from a fresh directory. The companion consumes a bundle from a separate execution (upload or `ARTIFACT_ZIP_PATH`, with a trusted whole-archive SHA-256 required before the Python-serialised predictor is loaded), validates it before reconstruction, validates new unlabelled rows into an input manifest, predicts labels with class probabilities, and writes a `not-measurable` evaluation report. See `tutorials/README.md` for the registry, `docs/WEIGHTS.md` for the snapshot layout and `docs/release-verification.md` for the release gate.

**Comparative classification workshop:** [`tutorials/DIMER_FreshRetailNet_MultiModel_Classification_Workshop.ipynb`](tutorials/DIMER_FreshRetailNet_MultiModel_Classification_Workshop.ipynb) compares Mitra, TabDPT v1.2, TabPFN-3 and TabICLv2 against heuristic and classical baselines on the pinned `freshretailnet-band-h7` low/mid/high demand-band sample. It is a separate standalone workshop carrier with notebook-embedded public adapters, not part of the generated standalone pair or its parity/release evidence. See [`tutorials/README.md`](tutorials/README.md) for its dataset hash, evaluation protocol, and current clean-runtime validation status.

**No DIMER Workbench access is required.** User data is processed in the notebook runtime, not by DIMER.

## Release status

**Candidate.** Static/unit checks do not constitute clean-runtime notebook evidence. The previous repository-installing pair was executed in clean hosted runners; the standalone pair has not been executed yet. Complete `docs/release-verification.md` against the exact release revision before calling either notebook release-grade.
