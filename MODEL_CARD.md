...
# Mitra Classifier — Tabular Foundation Model (Classification)
...
- **End-to-End Pipeline Tutorial**: \
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kurtvalcorza/mitra-classifier-pipeline/blob/main/tutorials/mitra_classifier_colab.ipynb) [`mitra_classifier_colab.ipynb`](https://github.com/kurtvalcorza/mitra-classifier-pipeline/blob/main/tutorials/mitra_classifier_colab.ipynb) \
...
- **Predictor Artifact Inference Tutorial**: \
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kurtvalcorza/mitra-classifier-pipeline/blob/main/tutorials/mitra_classifier_predictor_inference_colab.ipynb) [`mitra_classifier_predictor_inference_colab.ipynb`](https://github.com/kurtvalcorza/mitra-classifier-pipeline/blob/main/tutorials/mitra_classifier_predictor_inference_colab.ipynb) \
  *Validate an externally supplied predictor bundle and its trusted archive digest, load it without refitting, validate new unlabelled rows, and export class labels and probabilities with provenance.*
...
At inference the model conditions on the labelled training table as in-context support and emits a class-probability vector per query row; adaptation happens through in-context conditioning by default and, in this pipeline, through gradient fine-tuning when `fine_tune=true` (the default in `finetuner/train.py`). What this repository adds is the code around those weights: a dataset validator (`validator/`), a fine-tuner that verifies the checkpoint digests before loading when the weights come from the Hub, enforces Mitra's row and class ceilings, seeds every RNG, evaluates on a held-out split, and writes the result, provenance, and context artifacts (`finetuner/`), plus the contract documents and Colab tutorial. The upstream weights are not modified by this repository.
...
Concrete application domains envisioned during development: binary and multiclass risk or category classification, demand-class and churn prediction, quality-grade classification, event classification, and scientific or research classification represented as feature tables — in particular the small-data regime, where the upstream authors report the model strongest (below roughly 5,000 samples and 100 features). The pipeline is meant to play the role of a strong zero-configuration baseline or a fine-tuned production model for tables that fit the ceilings: at most 10,000 training rows (`MITRA_ROW_LIMIT`; larger tables are class-preservingly sampled down), at most 500 features (`MITRA_FEATURE_LIMIT`, validator check `feature_limit`), and at most 10 target classes (`MITRA_CLASS_LIMIT`). Time-series, transactional, sensor, or panel data must first be represented as a supervised feature table; Mitra is not a forecasting model.
...
**Operating environment.** The fine-tuner runs on the `pytorch:2.8.0-cuda12.8` base image with AutoGluon 1.5.0; training expects a CUDA device, and the torch build is pinned by the image so that sm_120 (RTX 50-series) hosts keep the cu128 wheel. Precision follows AutoGluon's Mitra defaults. The validator is CPU-only. Fine-tuning under AutoGluon 1.5.0 is seeded (`_seed_everything`) but not guaranteed bit-deterministic.
...
The fine-tuner evaluates the trained predictor on the held-out split with AutoGluon's `predictor.evaluate(..., auxiliary_metrics=True)` and writes every returned metric under `metrics.valEvaluation` in `result.json`, with `log_loss` sign-flipped to its conventional lower-is-better form. The headline metric is the `eval_metric` hyperparameter (default `accuracy`; `log_loss` and `roc_auc` map to Mitra-native early-stopping metrics, other AutoGluon metric names are reported but do not steer early stopping), recorded as `headlineMetric`/`headlineScore`.
...
The pipeline is not intended for decisions in health care, physical safety, criminal justice, legal rights, employment, credit, insurance, education access, or public benefits, and it has not been validated for any of them. The only validation performed is the contract testing in `scripts/` and the holdout evaluation on the operator's own table; no clinical, regulatory, or independent domain validation has been carried out by the developers or by any external body, and general benchmark performance is not evidence of suitability.
...
- **Refusals:** `classNames` is written on every result payload, success or failure, because the pipeline contract (`DIMER_CONTRACT.md`) requires it; the fine-tuner trains a single Mitra model with `fit_weighted_ensemble=False` and asserts that the requested model actually trained, so no silent fallback to another AutoGluon learner can occur.
...
- any use that violates the Apache-2.0 terms of the upstream `autogluon/mitra-classifier` weights and AutoGluon code, or the terms of the deployment that runs the pipeline.
...
> **Weights without `config.json`:** If a copy of the Mitra weights carries only the `model.safetensors` binary, recreate the configuration: because AutoGluon requires `config.json` in the same directory to initialize the Transformer architecture, you can recreate `weights/config.json` alongside `model.safetensors` using the exact text above:
> 
> ```bash
